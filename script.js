...
const modalCloseButton = projectModal ? projectModal.querySelector('.modal-close') : null;
const modalOverlay = projectModal ? projectModal.querySelector('.modal-overlay') : null;
const modalImageWrapper = projectModal ? projectModal.querySelector('.modal-image-wrapper') : null;
const modalProjectImage = projectModal ? projectModal.querySelector('.modal-project-image') : null;
const modalProjectTitle = projectModal ? projectModal.querySelector('.modal-project-title') : null;
const modalProjectSubtitle = projectModal ? projectModal.querySelector('.modal-project-subtitle') : null;
const modalProjectDescription = projectModal ? projectModal.querySelector('.modal-project-description') : null;
const modalAchievementsHeading = projectModal ? projectModal.querySelector('.modal-achievements-heading') : null;
const modalAchievementsList = projectModal ? projectModal.querySelector('.modal-achievements-list') : null;
const modalButtonsContainer = projectModal ? projectModal.querySelector('.modal-buttons') : null;
const modalViewPaperButton = projectModal ? projectModal.querySelector('.modal-view-paper') : null;
const modalExploreProjectButton = projectModal ? projectModal.querySelector('.modal-explore-project') : null;
const modalSkillContent = projectModal ? projectModal.querySelector('.modal-skill-content') : null;
const skillModalStarsContainer = projectModal ? projectModal.querySelector('.skill-modal-stars') : null;
const modalSkillProjectsGrid = projectModal ? projectModal.querySelector('.modal-skill-projects-grid') : null;
const modalSkillProjectsHeading = projectModal ? projectModal.querySelector('.modal-skill-projects-heading') : null;
const modalSkillToolsList = projectModal ? projectModal.querySelector('.modal-skill-tools-list') : null;
const modalSkillToolsHeading = projectModal ? projectModal.querySelector('.modal-skill-tools-heading') : null;
...
function getSearchableItems() {
    ...
    // Add Projects
    portfolioData.projects.forEach(p => items.push({ 
        id: p.id, 
        text: p.title, 
        type: 'Project',
        searchTerms: `${p.title.toLowerCase()} ${p.subtitle ? p.subtitle.toLowerCase() : ''} ${p.description.toLowerCase()}`
    }));
    
    // Add Skills
    portfolioData.skills.forEach(s => items.push({ 
        id: s.id, 
        text: s.title, 
        type: 'Skill',
        searchTerms: `${s.title.toLowerCase()} ${s.description.toLowerCase()}`
    }));
    
    // Add Experience
    portfolioData.experience.forEach(e => items.push({ 
        id: e.id, 
        text: e.title, 
        type: 'Experience',
        searchTerms: `${e.title.toLowerCase()} ${e.subtitle.toLowerCase()}`
    }));

    // Add Leadership
    portfolioData.leadership.forEach(l => items.push({ 
        id: l.id, 
        text: l.title, 
        type: 'Leadership',
        searchTerms: `${l.title.toLowerCase()} ${l.organization.toLowerCase()} ${l.summary.toLowerCase()}`
    }));

    // Add Main Sections (for direct navigation)
    ['home', 'about', 'projects', 'experience', 'skills', 'leadership', 'contact'].forEach(id => {
        items.push({ id: id, text: id.charAt(0).toUpperCase() + id.slice(1), type: 'Section', searchTerms: id });
    });
...
function updateDropdown(query) {
    if (!suggestionsList || !searchDropdown) return;
    
    suggestionsList.innerHTML = '';
    
    if (query.length === 0) {
        renderRecentSearches();
        searchDropdown.style.display = recentSearches.length > 0 ? 'block' : 'none';
        return;
    }

    const lowerCaseQuery = query.toLowerCase();
    const matches = searchableItems.filter(item => item.searchTerms.includes(lowerCaseQuery));
    
    if (matches.length > 0) {
        // Limit suggestions to 10
        matches.slice(0, 10).forEach((item) => {
            const li = document.createElement('li');
            li.setAttribute('data-target-id', item.id);
            li.setAttribute('data-target-type', item.type);
            li.innerHTML = `${item.text} <span class="result-type">${item.type}</span>`;
            suggestionsList.appendChild(li);
        });
        
        searchDropdown.style.display = 'block';
        
        // Ensure Recent Searches header/list are hidden when suggestions are active
        const recentTitle = document.querySelector('.search-dropdown-content .dropdown-section-title:nth-of-type(1)');
        const suggestionsTitle = document.querySelector('.search-dropdown-content .dropdown-section-title:nth-of-type(2)');
        
        if (recentTitle) recentTitle.style.display = 'none';
        if (recentSearchesList) recentSearchesList.style.display = 'none';
        if (suggestionsTitle) suggestionsTitle.style.display = 'block';
...
function renderRecentSearches() {
    if (!recentSearchesList || !searchDropdown) return;
    ...
    if (recentSearches.length > 0) {
        ...
        if (suggestionsTitle) suggestionsTitle.style.display = 'none';
        if (suggestionsList) suggestionsList.style.display = 'none';
        
        recentSearches.slice(0, 5).forEach(search => {
            const li = document.createElement('li');
            li.textContent = search;
            li.setAttribute('data-is-recent', 'true');
...
function handleSearchExecution(targetId) {
    if (!targetId) {
        const query = searchInput.value.trim();
        const firstMatch = searchableItems.find(item => item.searchTerms.includes(query.toLowerCase()));
        targetId = firstMatch ? firstMatch.id : null;
    }
    
    if (targetId) {
        const item = searchableItems.find(i => i.id === targetId);
        saveSearch(item ? item.text : searchInput.value);
...
function performContentFilter(query) {
    ...
    allCards.forEach(card => {
        ...
        if (item && item.searchTerms.includes(lowerCaseQuery)) {
             found = true;
        }

        // Keep content visible if query is empty OR if the card matches the query
...
// Modal Listeners
if (modalCloseButton) modalCloseButton.addEventListener('click', closeGenericModal);
if (modalOverlay) modalOverlay.addEventListener('click', closeGenericModal);
...
if (searchInput) {
    searchInput.addEventListener('input', (event) => {
        ...
        performContentFilter(query);
    });

    searchInput.addEventListener('keydown', (event) => {
        const suggestions = suggestionsList ? suggestionsList.querySelectorAll('li') : [];
        if (!searchDropdown || searchDropdown.style.display !== 'block' || suggestions.length === 0) return;
        ...
        if (event.key === 'ArrowDown') {
            ...
            if (currentSuggestionIndex !== -1) {
                ...
                handleSearchExecution(selectedItem.getAttribute('data-target-id'));
            } else {
...
if (searchDropdown) {
    searchDropdown.addEventListener('click', (event) => {
        ...
        if (li) {
            if (li.hasAttribute('data-is-recent')) {
                searchInput.value = li.textContent;
                // Re-filter the content cards based on the populated search term
                performContentFilter(li.textContent); 
                updateDropdown(li.textContent);
            } else {
                handleSearchExecution(li.getAttribute('data-target-id'));
            }
        }
    });
}
...
document.addEventListener('click', (event) => {
    if (searchInput && !event.target.closest('.search-container') && searchDropdown) {
        searchDropdown.style.display = 'none';
    }
});


// Navigation/Utility Listeners
if (darkModeToggle) {
...
if (hamburgerMenu) {
    hamburgerMenu.addEventListener('click', () => {
        if (mainNavUl) mainNavUl.classList.toggle('show');
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. Initialize Searchable Items (must be done after data is loaded)
